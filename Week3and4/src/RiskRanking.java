...
public class RiskRanking {
    ...
    // Utility to copy array (important for clean results)
    static Client[] copyArray(Client[] original) {
        Client[] copy = new Client[original.length];
        for (int i = 0; i < original.length; i++) {
            copy[i] = new Client(
                    original[i].name,
                    original[i].riskScore,
                    original[i].accountBalance
            );
        }
        return copy;
    }
    ...
    public static void main(String[] args) {
        ...
        // Use copies so sorting doesn't overwrite original order
        Client[] asc = copyArray(clients);
        Client[] desc = copyArray(clients);

        bubbleSort(asc);        // ASC
        insertionSort(desc);    // DESC
        topRiskClients(desc);   // Top risks (from DESC sorted array)
    }
}